...
async def get_routing(admin: CurrentAdmin):
    """Current exchange→account routing config + LIVE failover status
    (per-account health + effective routing so the UI can show when a
    failover is active)."""
    ...
    if cfg is None:
        ...
        try:
            ...
            logger.debug("zerodha_routing_seed_failed", exc_info=True)
    return {
        "success": True,
        "config": {
            "exchange_account_map": cfg.exchange_account_map or DEFAULT_EXCHANGE_ACCOUNT_MAP,
        ...
        "live": zerodha.failover_status(),
    }


@router.put("/routing")
async def update_routing(payload: dict[str, Any], admin: CurrentAdmin):
    """Update the exchange→account routing map + failover knobs. Keys of
    ``exchange_account_map`` are Kite exchange codes; values must be 0 (A) or
    1 (B). Invalid values are dropped."""
    from app.models.zerodha_feed_routing import (
        DEFAULT_EXCHANGE_ACCOUNT_MAP,
        ZerodhaFeedRouting,
    )
    ...
    raw_map = payload.get("exchange_account_map")
    if isinstance(raw_map, dict):
        cleaned: dict[str, int] = {}
        for k, v in raw_map.items():
            ex = str(k or "").strip().upper()
            if not ex:
                continue
            try:
                acct = int(v)
            ...
            if acct in (0, 1):
                cleaned[ex] = acct
        cfg.exchange_account_map = cleaned or DEFAULT_EXCHANGE_ACCOUNT_MAP.copy()

    if "failover_enabled" in payload:
        cfg.failover_enabled = bool(payload.get("failover_enabled"))
    for fld, lo, hi in (
        ("failover_confirm_down_sec", 1, 120),
        ("failback_confirm_up_sec", 1, 600),
    ):
        if payload.get(fld) not in (None, ""):
            try:
                setattr(cfg, fld, max(lo, min(hi, int(payload[fld]))))
            except (TypeError, ValueError):
                pass
    ...
    if cfg.id is None:
        ...
        await cfg.save()
    # Push the new config into the running failover controller immediately
    # (it also refreshes every ~3s, but this makes the UI feel instant).
    try:
        await zerodha._refresh_routing_config()
    except Exception:
        logger.debug("zerodha_routing_hot_refresh_failed", exc_info=True)
    return {
        "success": True,
        "config": {
...
async def disconnect_account(
    ...
    """Tear down ONLY one account's WebSocket (the other keeps streaming).
    The failover controller re-routes the dropped account's tokens onto the
    survivor within a couple seconds. Handy for testing failover."""
