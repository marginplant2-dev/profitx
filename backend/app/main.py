...
async def lifespan(_: FastAPI) -> AsyncIterator[None]:
    ...
    async def _feed_leader_main() -> None:
        ...
        try:
            ...
            # Dual-account HA failover controller — keeps each exchange's
            # tokens on its routed Kite account (A=NSE/BSE, B=MCX) and moves
            # them to the surviving account when one socket drops, so the feed
            # never stops. Leader-only (drives the shared WS pool). See
            # zerodha_service.feed_failover_loop + models/zerodha_feed_routing.
            subtasks.append(
                _asyncio.create_task(
                    _supervise(
