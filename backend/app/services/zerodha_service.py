...
class ZerodhaService:
    ...
    def __init__(self) -> None:
        ...
        # ── Dual-account routing + HA failover state ─────────────────────
        # See models/zerodha_feed_routing.py + feed_failover_loop below.
        # api_key -> account_index (0=A, 1=B) and its inverse, refreshed from
        # ZerodhaSettings by the failover loop so we can tell which pool entry
        # belongs to which account.
        self._api_key_to_account: dict[str, int] = {}
        self._account_api_key: dict[int, str] = {}
        # Cached routing config (Kite exchange -> account index) + knobs.
        self._routing_map: dict[str, int] = {}
        self._failover_enabled: bool = True
        self._failover_confirm_down_sec: int = 5
        self._failback_confirm_up_sec: int = 25
        # Hysteresis bookkeeping (monotonic seconds): when each account was
        # first seen up/down, and its debounced "effective up" state — so a
        # flapping socket never thrashes token re-subscriptions.
        self._account_up_since: dict[int, float] = {}
        self._account_down_since: dict[int, float] = {}
        self._account_effective_up: dict[int, bool] = {}
        # Exchange -> effective account index, recomputed each failover tick
        # and read by _ws_subscribe so NEW tokens land on the right account.
        self._effective_account_by_exchange: dict[str, int] = {}
    ...
    async def get_status(self, account_index: int = 0) -> dict[str, Any]:
        ...
        # The WS pool lives on the feed-leader worker; the admin API runs on
        # non-leader workers whose local `self._tickers` is empty. Prefer the
        # leader's Redis snapshot so each account (esp. B) shows its TRUE
        # ticker state + token count instead of a permanent 'DISCONNECTED'.
        snap_connected, snap_tokens, snap_err = await self._read_account_ws_snapshot(
            account_index
        )
        b_last_close_reason = ""
        if snap_connected is not None:
            ws_status_str = (
                WsStatus.CONNECTED.value if snap_connected else WsStatus.DISCONNECTED.value
            )
            b_last_close_reason = snap_err
        elif account_index != 0 and s.apiKey:
            # No snapshot (single-worker / running on the leader) — read the
            # local pool. _async_set_status only writes Account A's doc, so B
            # can't use the DB wsStatus.
        ...
        # Per-account subscribed count from the leader snapshot when available;
        # else the local pool's total (correct only on the leader).
        subscribed_count = (
            snap_tokens if snap_tokens is not None else pool["total_tokens_subscribed"]
        )
    ...
    async def get_settings_full(self, account_index: int = 0) -> dict[str, Any]:
        ...
        # Prefer the feed-leader's Redis snapshot (the WS pool isn't on this
        # worker). Only Account A's wsStatus is persisted to the DB, so B must
        # not fall back to the DB value.
        snap_connected, _snap_tokens, snap_err = await self._read_account_ws_snapshot(
            account_index
        )
        if snap_connected is not None:
            ws_status_str = (
            ...
            ws_last_error = snap_err or s.wsLastError
        elif account_index != 0 and s.apiKey:
            with self._ticker_lock:
                b_entry = next(
                    (e for e in self._tickers if e.get("api_key") == s.apiKey),
    ...
    async def _account_ws_connect(self, api_key: str, access_token: str) -> None:
        """Spawn (or reconnect) ONE account's dedicated WebSocket slot.

        Tears down ONLY that account's stale entries (matched by api_key) —
        the OTHER account's socket + its live tokens are left completely
        untouched. This is the HA-safe reconnect: Account A can be recovered
        without dropping Account B (and vice-versa), so a single account's
        socket flap never stops the whole feed. Orphaned tokens from the
        dropped slot are re-subscribed onto a healthy account by
        ``feed_failover_loop`` within a couple of seconds.
        """
        ...
        with self._ticker_lock:
            ...
            # Rebuild token->index map from the survivors so stale indices
            # (which pointed at the dropped slot) can't mis-route the
            # reconcile loop.
            self._token_to_ws = {}
            for i, e in enumerate(self._tickers):
                for tok in (e.get("tokens") or set()):
        ...
        try:
            ...
            logger.info(
                "zerodha_account_ws_spawned",
                extra={"account": self._api_key_to_account.get(api_key)},
            )
        except Exception:
            logger.exception("zerodha_account_ws_spawn_failed")

    async def _account_b_ws_connect(self, api_key: str, access_token: str) -> None:
        """Back-compat wrapper — Account B reconnect via the generalised,
        account-scoped connect (see ``_account_ws_connect``)."""
        await self._account_ws_connect(api_key, access_token)
    ...
    # ── Dual-account routing helpers ────────────────────────────────────
    def _exchange_of_token(self, token: int) -> str:
        """Kite exchange code (NSE/BSE/NFO/BFO/MCX/...) for a token, upper-cased.
        Empty string when unknown → routes to Account A by default."""
        info = self._symbol_by_token.get(int(token))
        if info:
            return str(info.get("exchange") or "").upper()
        return ""

    def _target_account_for_token(self, token: int) -> int:
        """Which account (0=A / 1=B) this token SHOULD stream on right now —
        the failover controller's effective routing wins, then the static
        map, then Account A."""
        ex = self._exchange_of_token(token)
        if ex:
            if ex in self._effective_account_by_exchange:
                return self._effective_account_by_exchange[ex]
            if ex in self._routing_map:
                return self._routing_map[ex]
        return 0

    def _entry_account_locked(self, entry: dict[str, Any]) -> int:
        """Account index that owns a pool entry (by api_key). Call under
        ``_ticker_lock``. Unknown api_key → Account A (0)."""
        return self._api_key_to_account.get(entry.get("api_key") or "", 0)

    def _pick_ws_idx_locked(self, preferred_account: int | None) -> int:
        """Index of the least-loaded CONNECTED WS with room. Prefer entries
        owned by ``preferred_account``; if that account has no room/socket,
        fall back to ANY connected entry (implicit failover so the feed never
        stalls while one socket is alive). -1 when nothing has capacity. Call
        under ``_ticker_lock``."""

        def _least_loaded(only_account: int | None) -> int:
            best_idx = -1
            best_count = self.MAX_TOKENS_PER_WS + 1
            for i, entry in enumerate(self._tickers):
                if not (
                    entry.get("connected")
                    and len(entry["tokens"]) < self.MAX_TOKENS_PER_WS
                ):
                    continue
                if only_account is not None and self._entry_account_locked(entry) != only_account:
                    continue
        ...
        if preferred_account is not None:
            idx = _least_loaded(preferred_account)
            if idx != -1:
                return idx
        return _least_loaded(None)

    def _ws_subscribe(self, tokens: list[int]) -> None:
        """Subscribe tokens on-demand — route each to its target account's
        least-loaded connected WS (dual-account exchange routing + failover),
        falling back to any connected WS so a token is never left un-streamed
        while at least one socket is alive."""
        with self._ticker_lock:
            if not self._tickers:
                return
            for token in tokens:
                ...
                target_acct = self._target_account_for_token(token)
                best_idx = self._pick_ws_idx_locked(target_acct)
    ...
    # ── Dual-account HA failover controller ─────────────────────────────
    async def _refresh_routing_config(self) -> None:
        """Reload the exchange→account routing map + failover knobs from
        ``ZerodhaFeedRouting`` (seed default on first miss) and refresh the
        api_key↔account_index maps from both accounts' ``ZerodhaSettings``.
        ...
        try:
            ...
            if cfg is None:
                ...
                try:
                    ...
                    pass
            src = cfg.exchange_account_map or DEFAULT_EXCHANGE_ACCOUNT_MAP
            self._routing_map = {str(k).upper(): int(v) for k, v in src.items()}
            self._failover_enabled = bool(cfg.failover_enabled)
            self._failover_confirm_down_sec = int(cfg.failover_confirm_down_sec or 5)
            self._failback_confirm_up_sec = int(cfg.failback_confirm_up_sec or 25)
        except Exception:
            logger.debug("zerodha_routing_config_refresh_failed", exc_info=True)
        try:
            a2k: dict[int, str] = {}
            k2a: dict[str, int] = {}
            for idx in (0, 1):
                s = await ZerodhaSettings.find_one(ZerodhaSettings.account_index == idx)
                if s and s.apiKey:
                    a2k[idx] = s.apiKey
                    k2a[s.apiKey] = idx
            self._account_api_key = a2k
            self._api_key_to_account = k2a
        except Exception:
            logger.debug("zerodha_account_key_refresh_failed", exc_info=True)

    def _account_connected_raw(self, account_index: int) -> bool:
        """True if this account currently has a CONNECTED socket in the pool."""
        api_key = self._account_api_key.get(account_index)
        if not api_key:
            return False
        with self._ticker_lock:
            return any(
                e.get("connected") and e.get("api_key") == api_key for e in self._tickers
            )

    def account_healthy(self, account_index: int) -> bool:
        """Debounced health (effective_up) — the failover controller's view of
        whether an account is usable. Falls back to the raw connected flag
        before the first controller tick."""
        if account_index in self._account_effective_up:
            return self._account_effective_up[account_index]
        return self._account_connected_raw(account_index)

    def resolve_target_account(self, exchange: str) -> int:
        """Account an exchange's tokens should stream on right now: its
        configured account if healthy, else the surviving account, else the
        configured account (both down → self-heal recovers it)."""
        desired = self._routing_map.get((exchange or "").upper(), 0)
        if self.account_healthy(desired):
            return desired
        other = 1 - desired
        if self.account_healthy(other):
            return other
        return desired

    def _recompute_effective_health(self) -> None:
        """Update the debounced per-account health + the exchange→account
        effective routing table (hysteresis prevents a flapping socket from
        thrashing re-subscriptions)."""
        now = time.monotonic()
        for idx in (0, 1):
            present = idx in self._account_api_key
            raw = present and self._account_connected_raw(idx)
            if idx not in self._account_effective_up:
                self._account_effective_up[idx] = raw
                (self._account_up_since if raw else self._account_down_since)[idx] = now
                continue
            if raw:
                self._account_down_since.pop(idx, None)
                self._account_up_since.setdefault(idx, now)
                if not self._account_effective_up[idx] and (
                    now - self._account_up_since[idx] >= self._failback_confirm_up_sec
                ):
                    self._account_effective_up[idx] = True
                    logger.info("zerodha_account_failback", extra={"account": idx})
            else:
                self._account_up_since.pop(idx, None)
                self._account_down_since.setdefault(idx, now)
                if self._account_effective_up[idx] and (
                    now - self._account_down_since[idx] >= self._failover_confirm_down_sec
                ):
                    self._account_effective_up[idx] = False
                    logger.warning("zerodha_account_failover", extra={"account": idx})
        eff: dict[str, int] = {}
        for ex, desired in self._routing_map.items():
            if self._account_effective_up.get(desired):
                eff[ex] = desired
            elif self._account_effective_up.get(1 - desired):
                eff[ex] = 1 - desired
            else:
                eff[ex] = desired
        self._effective_account_by_exchange = eff

    def _reconcile_token_routing(self) -> None:
        """Move any subscribed token that is NOT on its effective account's
        connected socket onto the right one (the actual failover action).
        Idempotent — a steady, correctly-routed pool does nothing."""
        with self._ticker_lock:
            if not self._tickers:
                return
            for token, ws_idx in list(self._token_to_ws.items()):
                cur_entry = (
                    self._tickers[ws_idx] if 0 <= ws_idx < len(self._tickers) else None
                )
                target_acct = self._target_account_for_token(token)
                cur_ok = bool(
                    cur_entry
                    and cur_entry.get("connected")
                    and token in cur_entry.get("tokens", set())
                    and self._entry_account_locked(cur_entry) == target_acct
                )
                if cur_ok:
                    continue
                new_idx = self._pick_ws_idx_locked(target_acct)
                if new_idx == -1 or new_idx == ws_idx:
                    continue
                new_entry = self._tickers[new_idx]
                if cur_entry is not None:
                    cur_entry["tokens"].discard(token)
                    if cur_entry.get("connected"):
                        self._schedule_ws_send(cur_entry, {"a": "unsubscribe", "v": [token]})
                new_entry["tokens"].add(token)
                self._token_to_ws[token] = new_idx
                self._schedule_ws_send(new_entry, {"a": "subscribe", "v": [token]})
                self._schedule_ws_send(new_entry, {"a": "mode", "v": ["full", [token]]})

    async def _publish_account_ws_status(self) -> None:
        """Feed-leader: mirror each account's live WS status (connected +
        token count + last close reason) into Redis so ANY worker can render
        the TRUE per-account ticker state. The admin API runs on non-leader
        workers whose local `self._tickers` is empty — without this snapshot
        Account B always showed 'DISCONNECTED' off that empty pool."""
        try:
            from app.core.redis_client import cache_set

            snap: dict[int, dict[str, Any]] = {
                0: {"connected": False, "tokens": 0, "err": ""},
                1: {"connected": False, "tokens": 0, "err": ""},
            }
            with self._ticker_lock:
                for e in self._tickers:
                    idx = self._api_key_to_account.get(e.get("api_key") or "")
                    if idx not in (0, 1):
                        continue
                    if e.get("connected"):
                        snap[idx]["connected"] = True
                    snap[idx]["tokens"] += len(e.get("tokens") or set())
                    if e.get("last_close_reason"):
                        snap[idx]["err"] = e.get("last_close_reason")
            for idx in (0, 1):
                await cache_set(f"zerodha:acct_ws:{idx}", snap[idx], ttl_sec=20)
        except Exception:
            logger.debug("zerodha_acct_ws_status_publish_failed", exc_info=True)

    async def _read_account_ws_snapshot(
        self, account_index: int
    ) -> tuple[bool | None, int | None, str]:
        """(connected, tokens, err) from the feed-leader's Redis snapshot, or
        (None, None, "") when unavailable — the caller then falls back to the
        local pool (single-worker / on the leader itself)."""
        try:
            from app.core.redis_client import cache_get

            snap = await cache_get(f"zerodha:acct_ws:{account_index}")
            if snap is not None:
                return (
                    bool(snap.get("connected")),
                    int(snap.get("tokens") or 0),
                    str(snap.get("err") or ""),
                )
        except Exception:
            pass
    ...
    async def feed_failover_loop(self, interval_sec: float = 3.0) -> None:
        """HA controller (feed-leader only): keep every subscribed token on
        the socket of the account its exchange is routed to, failing tokens
        over to the surviving account when one goes down and back (with
        hysteresis) when it recovers — so the feed NEVER stops. Best-effort;
        a bad iteration is logged and skipped, never raised."""
        logger.info("zerodha_failover_loop_started")
        while True:
            try:
                await self._refresh_routing_config()
                self._recompute_effective_health()
                if self._failover_enabled:
                    self._reconcile_token_routing()
                await self._publish_account_ws_status()
            except asyncio.CancelledError:
                raise
            except Exception:
                logger.debug("zerodha_failover_loop_iter_failed", exc_info=True)
            await asyncio.sleep(interval_sec)

    def failover_status(self) -> dict[str, Any]:
        """Live snapshot for the admin Routing/Failover UI: per-account health
        + the current effective exchange→account routing (shows when a
        failover is active)."""
        accounts = {}
        for idx in (0, 1):
            accounts[str(idx)] = {
                "configured": idx in self._account_api_key,
                "connected": self._account_connected_raw(idx),
                "effective_up": self._account_effective_up.get(idx),
            }
        return {
            "failover_enabled": self._failover_enabled,
            "routing_map": self._routing_map,
            "effective_routing": self._effective_account_by_exchange,
            "accounts": accounts,
        }

    async def disconnect_account_ws(self, account_index: int) -> None:
        """Admin: tear down ONLY one account's socket (leaving the other
        account streaming). Its tokens are orphaned and the failover loop
        moves them to the surviving account within a couple seconds."""
        api_key = self._account_api_key.get(account_index)
        if not api_key:
            # Fall back to resolving from settings if the cache is cold.
            try:
                s = await self._get_settings(account_index)
                api_key = s.apiKey or None
            except Exception:
                api_key = None
        if not api_key:
            return
        ...
        for t in stale_tasks:
            t.cancel()
        logger.info("zerodha_account_ws_disconnected", extra={"account": account_index})
            ...
            while self._self_heal_running:
                try:
                    ...
                    if expiry and now_utc() >= expiry:
                        ...
                        await self._maybe_trigger_auto_login_when_token_missing()
                        continue
                    # ── Independent Account B self-heal ──────────────────
                    # Runs EVERY iteration (NOT gated on Account A being up) so
                    # Account B recovers even while A is down — true HA: neither
                    # account depends on the other's socket. Account-scoped
                    # reconnect leaves A untouched.
                    try:
                        ...
                        if s_b.apiKey and s_b.accessToken and s_b.isConnected:
                            ...
                            if not b_ok:
                                ...
                                await self._account_ws_connect(s_b.apiKey, s_b.accessToken)
                    except Exception:
                        logger.exception("zerodha_account_b_self_heal_failed")

                    # Account A health — check A's OWN socket (by api_key), NOT
                    # "any connected" (which would read True off a live B and
                    # hide a dead A).
                    with self._ticker_lock:
                        a_connected = any(
                            e.get("connected")
                            and (not s.apiKey or e.get("api_key") == s.apiKey)
                            for e in self._tickers
                        )
                    if a_connected:
                        continue
                    ...
                    try:
                        # Reconnect Account A. If Account B is currently
                        # streaming, use the account-scoped connect so B is NOT
                        # torn down (connect_ws(force=True) clears the WHOLE
                        # pool). With no live B to preserve, the full reset is
                        # fine and keeps the proven 403-escape sleep.
                        b_key = None
                        try:
                            _s_b2 = await self._get_settings(1)
                            b_key = _s_b2.apiKey or None
                        except Exception:
                            b_key = None
                        b_alive = False
                        if b_key:
                            with self._ticker_lock:
                                b_alive = any(
                                    e.get("connected") and e.get("api_key") == b_key
                                    for e in self._tickers
                                )
                        if b_alive and s.apiKey and s.accessToken:
                            await self._account_ws_connect(s.apiKey, s.accessToken)
                        else:
                            await self.connect_ws(force=True)
                        self._ws_consecutive_heal_failures = 0
